function f = lifting(cool_mat,l,grid_size)

    nodeN = length(cool_mat);
    ...
    Area1 = just_area(loc_mat_1,l,nodeN,grid_size);
    
    loc_mat_for_area_1 = loc_mat_for_volume(loc_mat_1,grid_size,l);
    loc_mat_for_area_2 = loc_mat_for_volume(loc_mat_2,grid_size,l);
    
    A_1_area = adjacency(loc_mat_for_area_1);
    A_2_area = adjacency(loc_mat_for_area_2);
    ...
    for i = 1:length(loc_mat_2)
        ...
        if (~isempty(areas))
            f(i) = equivalent_permeability(Area1(c_index_in_coolmat1),cool_mat_1(c_index_in_coolmat1,3),areas);
        end
