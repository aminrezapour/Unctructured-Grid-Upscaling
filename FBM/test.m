...
for i = 1:length(upperTriangLinks)
    ...
    linkWeight(i) = 10*weight; %gridblock is 5*5*5
end
...
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Fine Scale\conn.in','w');
fprintf(fileID,[num2str(counter0) '\n']);
fprintf(fileID,text);
fclose(fileID);

%writing the volume file
vol = 125*ones(length(A_T),1);
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Fine Scale\volume.in','w');
for i = 1:length(A_T)
    fprintf(fileID,'%f\n',vol(i)); 
end
fclose(fileID);
...
%% removing nodes
epsilon = 200; % this can be determined more intuitively
...
upper_bound = loc_mat_for_area(loc_mat_for_area(:,2)>63,:);
...
right_bound = loc_mat_for_area(loc_mat_for_area(:,1)>63,:);
...
loc_mat_for_area = [loc_mat_for_area;[0 0];[65 65];[0 65];[65 0]];
...
for i = 1:length(A)
    ...
    for j = 1:length(neighborNodesI)
        ...
        if c1==1 || length(c2)<1 || c2==1
            ...
            Aij = 5*h*norm(N_orth);
            c0 = 5*(v(c1,:)+v(c2,:))/2;
            Fi = 5*loc_mat_for_area(i,1:2)-c0;
            Fj = 5*loc_mat_for_area(neighborNodesI(j),1:2)-c0;
            Di = norm(Fi);
            Dj = norm(Fj);
...
for i = 1 : size(c ,1)
    ind = c{i}';
    area_i = polyarea( 5*v(ind,1) , 5*v(ind,2) );
    if area_i>0 
...
fclose(fileID);
text = fileread('conn_temp.in');
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 1\conn.in','w');
fprintf(fileID,[num2str(counter1) '\n']);
fprintf(fileID,text);
fclose(fileID);

%writing the volume file
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 1\volume.in','w');
for i = 1:nodeN
    fprintf(fileID,'%f\n',tess_area(i));
end
fclose(fileID);
...
fclose(fileID);
text = fileread('conn_temp.in');
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 2\conn.in','w');
fprintf(fileID,[num2str(counter1) '\n']);
fprintf(fileID,text);
fclose(fileID);

%writing the volume file
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 2\volume.in','w');
for i = 1:nodeN
    fprintf(fileID,'%f\n',tess_area(i));
end
fclose(fileID);
...
%% removing nodes
epsilon = 500; % this can be determined more intuitively
high_pass_nodes = odd_index(abs(D)>epsilon);
new_F = F;
new_F(high_pass_nodes) = 1;
new_F(1:64) = 1;
...
fclose(fileID);
text = fileread('conn_temp.in');
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 3\conn.in','w');
fprintf(fileID,[num2str(counter1) '\n']);
fprintf(fileID,text);
fclose(fileID);

%writing the volume file
fileID = fopen('D:\Dropbox\USC - Amin\matlab\FBM\GPRS\Coarse Scale 3\volume.in','w');
for i = 1:nodeN
    fprintf(fileID,'%f\n',tess_area(i));
end
fclose(fileID);